...
- `src/api/axios.ts` é o client HTTP único; um interceptor injeta `Authorization: Bearer <token>` do `localStorage`. **baseURL está hardcoded como `http://localhost:3000`** (não usa env var no client, apesar do `VITE_API_URL` documentado no README).
